...
public class AndroidLauncher extends AndroidApplication implements PlayServices,
	...
	public void broadcastMessage(String message) {
		...
		try {
			runOnUiThread(new Runnable() {
				...
				public void run() {
					boolean test = true;
					byte[] MsgBuf = msg.getBytes(Charset.forName("UTF-8"));
					...
					for (Participant p : mParticipants) {
						...
						if (test) {
							...
							Games.RealTimeMultiplayer.sendReliableMessage(gameHelper.getApiClient(), null, MsgBuf, mRoomId,
									p.getParticipantId());
						}
					}
				}
	...
	public void onPeerDeclined(Room room, List<String> peers) {
		...
		if (!mPlaying && shouldCancelGame(room)) {
			Games.RealTimeMultiplayer.leave(gameHelper.getApiClient(), null, mRoomId);
